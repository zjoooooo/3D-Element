...
### T7 回归 + 收官
- **三套件全绿**;`npm run sim` 四锚全中带内:新手 0% / 基线死亡中位 **14.0min** / 熟练 100% / 等级中位 **29**。
- **诚实限制(重要)**:`scripts/sim-run.mjs` **完全不读技能表**(grep 确认:无 ELEMENTS / ABILITY_TYPES / UpgradePool / loadout 引用),它是抽象统计模型。所以"带内"只说明**难度模型未被改动**,**不构成十技的平衡验证**。十技对 build 空间的真实影响(尤其新技卡在手牌中的占比上升——每个可施放且未拥有的元素各占一张候选,注册十技把新技候选从 ~13 抬到 ~23)**尚未被任何自动化手段覆盖**,留作后续里程碑的显式课题。
- 浏览器全项:十技逐个可施放 **10/10**、旧技抽查 **5/5**、沙盒 180 帧零错;各技专项在其任务轮已逐条验过(推退/雹幕/破甲/处决/落雷/龙息扇面/弹幕走位/拉向圆心/迟钝/抛飞晕)。
- README 增第二波十技表 + 两条教训段;CLAUDE.md 状态行与通道规则已更新。
- **未收的已知项(明账,交给后续里程碑)**:
  1. `damage()` 自带的基线冲量对 `zoneTick`/`lineTick` 同样逐 tick 施加(实测 snare 峰值 28.7 m/s、thornroad 19.1、beam 17.2)——M8 之前的老账,本里程碑的通道规则把它变成明账,但改动会触及多个已发布技能的手感,需单独立项。
  2. 第二波十技**无 breakpoints 表**(Lv3/Lv5 无质变),升级卡已按"该档存在才显示"处理,不漏裸键;若要给十技补质变节点是独立工作量。
  3. CRACK 族地贴的满尺寸窗与满 alpha 窗只在 45%-55% 重叠(GLSL 归一化性质,T3/T6 两处限时场同款),真要"第 0 秒画满"须动 shader。
  4. 扇形技仍沿用默认 LINE 施法指示器(画线不画扇);计划未要求新指示器。
  5. 雷暴 `age` 表达式里的 fade 项对当前数值是死代码(闸本身必要,但当前 8 道雷全落在 impact 窗内)。
