...
### 续作指引(2026-08-17 更新:T6 已完成,给下一个会话/下一步)

**从 Task 7(回归收官)开始。** T1-T6 已完成、已评审。T7 清单见计划正文(README 融合段/三套件+sim 复核/浏览器全项/勘误收口);注意 T7 清单中「淬炼对子系金融合生效」一行已在 T5 勘正(仅锋岩星阵)。sim 越带停下报数;完成后按环境交付要求推送 claude/* 分支。随后按用户 2026-08-17 指令:**编写 M8 计划并完整执行,再编写 M9 计划并完整执行**(计划入 docs/superpowers/plans/,体例与本文件一致:Global Constraints/数值表/逐任务断言先行/文末勘误节)。
