...
9. **小项（移交全分支终审裁量，留档）**：同帧升级+死亡时卡面先于判词弹出；死亡瞬间未清 `_echoAt`（判词后可能有一发纯 VFX 回响）；卡面摘要用元素原始 id 而卡片用 `label`；冷却环分母未乘修正（凝神下环起点 ~83%，读感"更快"成立）；升级卡文案 "伤害 +25%" 与 `damagePerLevel` 散文重复。
