...
执行完成于 2026-08-15，8 任务全部落地（commits db296d9..HEAD），双校验 + sim 全绿，沙盒逐帧不变。以下为计划文本与真实代码/自身约束的出入，均按"规程与真实代码为准"裁决：

1. **规程③ vs Task 1 的 settings 块**：计划让 Task 1 一次写入 11 个键，其中 4 个本任务无读者。已拆分：`skillLevelMax` 随 Task 2、`passiveWeights`/`milestones` 随 Task 3、`skipHeal` 随 Task 7 与各自读代码同任务落地。终态与计划一致。
2. **"六处伤害读数"实为 5+1**：计划正文枚举 5 处（sweep/burst 引爆/burst 灼烧/lineTick/zoneTick）；第 6 处是 `FireballAbility.js` 的自结算（`settings.combat.fireball = {kind:'self'}` 使 CombatSystem 跳过它）——fireball 在默认第 2 席，漏掉它升级卡即空话。已修：FireballAbility 乘 `this.ctx.mods?.damageMult(this.element) ?? 1`（沙盒恒 1），App 构造期挂 `abilities.ctx.mods`。Node 不可断言（THREE 绑定），浏览器实测飘字 20→25。
3. **`ELEMENT_META` 显示名字段是 `label` 不是 `name`**：计划自带的 grep 指引命中，UpgradeUi 按实调整。
4. **空手界面**：计划正文说"空手时单按钮"，但 Task 6 钉死代码在 rerolls>0 时仍渲染刷新钮。按正文执行：开空手界面时传 `rerolls: 0`。
5. **回响哑火洞（计划 Step 5 片段）**：`quickCast()` 对 ZONE 技能（snare/glacier）光标无效时直接退出，而片段先把冷却置 0——哑火后该技能永久免冷却。已加守卫：记录 prevCd，`===0` 信号（真实施放后冷却 ≥0.15s，且可哑火的两个元素冷却 1.4/1.6，余量 0.84s）复位。浏览器实测通过。
6. **Task 1 断言块漏 `xpMult()`**（违反规程②）：评审拦下，补一条精确值断言。
7. **`settings.run.loadout` 读点是 3 处不是"两处"**：计划 Step 2 正文写两处、括号却列了三处；全部改读 `loadout.elementAt(seat)`。
8. **冻结期冷却仍按 raw 递减**：计划"全停"枚举（走位/施法/VFX/敌人 + 三面门）不含冷却环，按枚举保留现状（冻结中无法施法，无利用面；与 M1 已记录的 P 暂停不对称同族）。若要严格全停，一行可改——留用户裁定。
终审（fable 全分支评审）后修复批次已落地：同帧死亡时判词先于卡面（回车即刻可用）、死亡即清 `_echoAt`、README 旧句"编辑器改席位即时生效"改为"下一局生效"（M2 快照语义）。终审对其余小项的裁量：3/5/9/10（四条廉价断言）排入 M3 开头；`has(null)` 隐患点名写进 M3 自动施法任务；余下接受。

9. **小项（终审裁量后留档）**：同帧升级+死亡时卡面先于判词弹出；死亡瞬间未清 `_echoAt`（判词后可能有一发纯 VFX 回响）；卡面摘要用元素原始 id 而卡片用 `label`；冷却环分母未乘修正（凝神下环起点 ~83%，读感"更快"成立）；升级卡文案 "伤害 +25%" 与 `damagePerLevel` 散文重复。
