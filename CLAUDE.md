...
2. **当前计划**:`docs/superpowers/plans/2026-08-17-m8-second-wave.md`(M8 第二波主动)— 文末「执行后勘误」是本项目最厚的一份,含通道规则、测试规则与五条明账未收项。历史计划 M1-M7 同目录,各自文末勘误是当时的真实偏差记录。
3. 数值即文档:`src/config/settings.js` 是活的调参台账。

## 当前状态(交接时更新此行)

v1.0 已发布(tag v1.0)。**M7 融合专属化已完成**(T1-T7)。**M8 第二波主动已完成**(T1-T7,十技全部注册;计划 `docs/superpowers/plans/2026-08-17-m8-second-wave.md`,勘误节末列有五条明账未收项)。用户指令的后续队列:**M9 → M10 → M11 → M12(需先写计划)→ 全任务复核**。
