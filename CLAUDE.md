...
2. **当前计划**:`docs/superpowers/plans/2026-08-18-m11-growth-and-boss.md`(M11 升级提速 + 首领战)— 文末「执行后勘误」含真实偏差与未收项。M10 的勘误立下**注入依赖的测试规则**,M8 的立下**通道规则与冻结规则**。历史计划 M1-M10 同目录,各自文末勘误是当时的真实偏差记录。
3. 数值即文档:`src/config/settings.js` 是活的调参台账。

## 当前状态(交接时更新此行)

v1.0 已发布(tag v1.0)。**M7 / M8 / M9 / M10 均已完成**(各自计划文末有勘误)。**M11 已完成**(T0 count 空头质变清账 / T1 升级提速三杠杆:每级两选+权重 3→6+经验 18/1.10 / T2 消掉 sim 的镜像并重标 / T3-T5 首领战:第四种行为、三幕三招、程序化躯体与奖励 / T6 收官重标;计划 `docs/superpowers/plans/2026-08-18-m11-growth-and-boss.md`)。用户指令的后续队列:**M12(需先写计划)→ 全任务复核**。
