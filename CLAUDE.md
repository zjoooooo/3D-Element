...
2. **当前计划**:`docs/superpowers/plans/2026-08-17-m9-draft-order.md`(M9 抽卡秩序/异化/无尽/通道清账)— 文末「执行后勘误」含各任务的真实偏差与未收项;M8 的勘误(同目录)立下了通道规则与测试规则。历史计划 M1-M8 同目录,各自文末勘误是当时的真实偏差记录。
3. 数值即文档:`src/config/settings.js` 是活的调参台账。

## 当前状态(交接时更新此行)

v1.0 已发布(tag v1.0)。**M7 融合专属化已完成**(T1-T7)。**M8 第二波主动已完成**(T1-T7,十技全部注册)。**M9 已完成**(T1 抽卡按类归一 / T2 满级异化 / T3 无尽续玩 / T4 damage() 通道清账 / T5 收官;计划 `docs/superpowers/plans/2026-08-17-m9-draft-order.md`)。用户指令的后续队列:**M10 → M11 → M12(需先写计划)→ 全任务复核**。
